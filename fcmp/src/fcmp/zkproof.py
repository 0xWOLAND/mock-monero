...
from .tree import Tree, root, path, hash_leaf, hash_node
...
def _pack(pp: Params, root_val: int, leaf: int, siblings: List[int], dirs: List[int], ctx: bytes) -> bytes:
    depth = len(siblings)
    sib_data = b"".join(to_bytes(s) for s in siblings)
    dir_data = bytes(dirs)
    path_data = depth.to_bytes(2, "big") + sib_data + dir_data
    binding = hashlib.sha256(b"ZK|bind|" + ctx + to_bytes(root_val) + to_bytes(leaf) + 
                           hashlib.sha256(path_data).digest()).digest()
...
def prove(pp: Params, tree: Tree, P: int, C: int, idx: int, ctx: bytes) -> ZKProof:
    root_val = root(tree)
...
def verify(pp: Params, root_val: int, P: int, C: int, proof: ZKProof, ctx: bytes) -> bool:
    ...
    siblings = [int.from_bytes(sib_data[i*32:(i+1)*32], "big") for i in range(depth)]
    dirs = list(dir_data)
    ...
    exp_binding = hashlib.sha256(b"ZK|bind|" + ctx + to_bytes(root_val) + to_bytes(leaf) +
                                hashlib.sha256(path_data).digest()).digest()
